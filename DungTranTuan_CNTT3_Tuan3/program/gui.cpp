...
int secondSelect(const MenuContext& firstMenu,  MenuContext& secondMenu, int  width, int height) {
    ...
    while (true) {
        ...
        drawBox(secondMenu.x, secondMenu.y-1 ,width + 24,height + secondMenu.options.size() + 2);
        // Gợi ý
        drawBox(MENU_X, MENU_Y + 12, 40, 4, 4);
        showSuggest();
...
int thirdSelect(MenuContext& firstMenu, MenuContext& secondMenu, MenuContext& thirdMenu, int  width, int height) {
    char key;
    while (true) {
        console::clearScreen();
        printLogo();
        drawingMenu(firstMenu.options, firstMenu.selected, firstMenu.x, firstMenu.y);
        drawBox(firstMenu.x, firstMenu.y-1 ,width + 24,height + firstMenu.options.size() + 2);// ve box
        drawingMenu(secondMenu.options, secondMenu.selected, secondMenu.x, secondMenu.y);
        ...
        // Gợi ý
        inputFrameArrange();//vẽ khung cho key arrange
        drawBox(MENU_X, MENU_Y + 12, 40, 4, 4);
        showSuggest();
...
void searchMenu(MenuContext& mainMenu, NhanVien *& danhSach) {
    ...
    while (true) {
        ...
        switch (choice) {
            case 0:
                keyMenu(mainMenu, danhSach, searchAlgos, 0);
                break;
            case 1:
                keyMenu(mainMenu, danhSach, searchAlgos, 1);
                break;
            default:
                console::gotoxy(MENU_X, MENU_Y + STATISTIC_OFFSET_Y + 22);
                cout << "Chuc nang dang phat trien..."; getch();
